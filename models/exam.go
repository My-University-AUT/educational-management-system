...
var getCourseExamByStudent = `
	SELECT exam_id, exam_name, start_date, end_date, duration, exam.course_id
	FROM exam, course_takes
	WHERE exam.course_id=?
	AND exam.course_id=course_takes.course_id
	AND course_takes.student_no=?
`

var getCourseQuestionsByStudent = `
	SELECT question_id, question_description, first_choice, second_choice, third_choice, fourth_choice, score, exam_id
	FROM exam_question
	WHERE exam.exam_id=?
`
...
func GetCourseExams(db *sql.DB, username, role, courseID string) ([]Exam, error) {

	var rows *sql.Rows
	var err error
	if role == "PROFESSOR" {
		rows, err = db.Query(getCourseExamByProf, courseID, username)
	} else {
		rows, err = db.Query(getCourseExamByStudent, courseID, username)
	}
